...
def get_public_ip_cpnat():
    try:
        with open('/var/cpanel/cpnat','r') as file:
            public_ip = file.read().split()[1]
        return public_ip
    except FileNotFoundError:  # In case this is not a cPanel server or not a nat server
        output = os.popen('curl https://myip.cpanel.net 2>/dev/null').read()
        public_ip = output.strip('\n')
        return public_ip
    

def ip_addresses():
    public_ip_nat = get_public_ip_cpnat()
    ip_list = {}
    ip_list['v4'] = {}
    ip_list['v6'] = {}
    ...
    if os.path.exists('/var/cpanel/cpnat'):
        ip_list['v4'][interface][0][0]['addr'] = public_ip_nat
    elif ip_list['v4'][interface][0][0]['addr'] != public_ip_nat:
        ip_list['v4'][interface][0][0]['addr'] = public_ip_nat
    else:
        pass
    return ip_list
